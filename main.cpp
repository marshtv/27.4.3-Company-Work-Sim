...
public:
	...
	explicit Employee(std::string in_name) {
		this->name = in_name;
	}
...
private:
	int managerID = 0;
public:
	Manager(std::string inName, int in_managerID) : Employee(inName) {
...
private:
	...
	Task* tasks = nullptr;
public:
	Team(int& in_ceoID, int in_teamID, std::string in_managerName, int in_managerID, int in_workersNum) {
		this->ceoID = in_ceoID;
		assert(in_teamID >= 0);
		...
		for (int i = 0; i < this->workersNum; ++i) {
			auto* worker = new Worker("Worker #" + std::to_string(i), Task::NONE);
...
public:
	CEO(std::string in_name, int in_teamNum, int in_workersNum) : Employee(in_name) {
		...
		for (int i = 0; i < this->teamsNum; ++i) {
			Team* team = new Team(this->ceoID, i,
								  "Manager #" + std::to_string(i), i, in_workersNum);
			this->teams[i] = team;
		}
	}
